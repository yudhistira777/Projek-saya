...
<?php } ?>
  ...
  <main>
    <div class="container">
<?php 
  $sql = "SELECT * FROM project WHERE deleted=0";

  if (!empty($cari)) {
    $cari = mysqli_real_escape_string($conn, $_GET['cari']);
    $sql .= " AND nama_project LIKE '%$cari%'";
  }

  $result = mysqli_query($conn, $sql) or die("query error: " . mysqli_error($conn));

  if (!mysqli_num_rows($result)) { ?>
      <p class="not-available">Maaf, proyek tidak tersedia!</p>
<?php } else while($proyek = mysqli_fetch_assoc($result)) { ?>
      <div class="column">
        <a class="card" href="detail-proyek.php?id=<?=htmlspecialchars($proyek['id_project'])?>">
          <div class="img-card">
            <img src="resources/img/gambar-proyek/<?=htmlspecialchars($proyek['gambar'])?>" alt="<?=htmlspecialchars($proyek['nama_project'])?>">
            <span>Rp<?=number_format($proyek['harga_project'], 0, ',', '.')?></span>
          </div>
          <p><?=htmlspecialchars($proyek['nama_project'])?></p>
        </a>
      </div>
<?php } ?>
    </div>
